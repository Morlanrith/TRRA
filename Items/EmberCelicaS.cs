...
namespace TRRA.Items
{
	...
	public class EmberCelicaS : ModItem
	{
		...
		public override void SetDefaults() {
			...
			item.UseSound = mod.GetLegacySoundSlot(SoundType.Item, "Sounds/Item/EmberShot");
		...
		public override bool CanUseItem(Player player) {
			if (player.altFunctionUse == 2) {
				...
				item.UseSound = mod.GetLegacySoundSlot(SoundType.Item, "Sounds/Item/EmberDash");
		...
		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
            if (player.altFunctionUse != 2)
            {
				...
				if (!player.mount.Active) Projectile.NewProjectile(position.X, position.Y, speedX * .25f, speedY * .25f, mod.ProjectileType("EmberPunch"), 180, 8, player.whoAmI);
			...
			else Projectile.NewProjectile(position.X, position.Y, speedX * .25f, speedY * .25f, mod.ProjectileType("EmberPunch"), 150, 12, player.whoAmI);

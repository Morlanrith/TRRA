	...
	public class GambolShroudS : ModItem
	{
		...
		public override bool CanUseItem(Player player)
		{
			if (player.altFunctionUse == 2)
			{
				...
				item.UseSound = mod.GetLegacySoundSlot(SoundType.Item, "Sounds/Item/ShadowClone");
				item.useTime = 30;
		...
		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			if (player.altFunctionUse != 2)
			{
				...
				Projectile.NewProjectile(position.X, position.Y, speedX, speedY, type, 50, knockBack, player.whoAmI, 30f, 0f);

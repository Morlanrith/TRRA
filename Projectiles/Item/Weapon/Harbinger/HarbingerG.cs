...
namespace TRRA.Projectiles.Item.Weapon.Harbinger
{
	public class HarbingerG : ModProjectile
	{
		...
		{
			if (Main.player[Projectile.owner].dead || Main.player[Projectile.owner].HeldItem.type != ItemType<Items.Weapons.HarbingerSw>() || Main.player[Projectile.owner].itemAnimation < 1)
			{
				Projectile.Kill();
				return;
			}
